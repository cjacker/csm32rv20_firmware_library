...
void System_Clock_Init2(void)//系统时钟初始化
{
    //时钟源开关
    CMU->SRC_EN  = 1<<1   //RCOSC    bit[1]:0-off, 1-on
                  |0<<0;  //crystal  bit[0]:0-off, 1-on

    //外设和内核时钟来源选择
	CMU->CLK_SEL = 0<<2   //peripheral  bit[3:2]:0-RCOSC, 1-crystal, 2-LSI(3K), 3-reserved
                  |0<<0;  //cpu         bit[1:0]:0-RCOSC, 1-crystal, 2-LSI(3K), 3-reserved
    ...
    //RC频率选择
    CMU_RC_DEFAULT->RC_DEFAULT = 1; //bit[0]:0-16MHz, 1-32MHz
